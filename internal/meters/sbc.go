...
func (p *SBCProducer) snip(iec Register, readlen uint16) Operation {
	opcode := p.regs[iec]
	return Operation{
...
func (p *SBCProducer) snip16(iec Register, scaler ...float64) Operation {
	snip := p.snip(iec, 1)
...
func (p *SBCProducer) snip32(iec Register, scaler ...float64) Operation {
	snip := p.snip(iec, 2)
...
func (p *SBCProducer) Produce() (res []Operation) {
	for _, op := range []Register{VoltageL1, VoltageL2, VoltageL1} {
		res = append(res, p.snip16(op))
	}

	for _, op := range []Register{CurrentL1, CurrentL2, CurrentL1} {
		res = append(res, p.snip16(op, 10))
	}

	for _, op := range []Register{
